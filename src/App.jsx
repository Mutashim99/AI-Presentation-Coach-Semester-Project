...
import {
  Activity, Mic, Cpu, AlertTriangle,
  ScanFace, Smile, Move, Terminal, Target, Download, Eye
} from 'lucide-react';

// --- CONFIGURATION ---
const GEMINI_API_KEY = "AIzaSyDqiqZfEpQiGmlFaOorIArMhnf80cH4TXU";
...
const App = () => {
  ...
  const [uiMetrics, setUiMetrics] = useState({
    wpm: 0, volume: 0, readingScore: 0, nervousness: 0,
    smileScore: 0, handActivity: 0, words: 0, fillers: 0,
    gazeStability: 100
  });

  const [uiDebug, setUiDebug] = useState({
    posture: 0, yaw: 0, pitch: 0, hand: 0
  });
  ...
  const metricsRef = useRef({
    wpm: 0, volume: 0, readingScore: 0, nervousness: 0,
    smileScore: 0, handActivity: 0, words: 0, fillers: 0,
    gazeStability: 100, fillerBreakdown: {}
  });
  ...
  const FILLERS = ['um', 'uh', 'like', 'actually', 'basically', 'literally', 'so', 'mean', 'you know', 'ahh', 'umm', 'uhh'];
  ...
  const captureBaseline = (pose, face) => {
    ...
    if (baselineRef.current.shoulderHeight === 0) {
      baselineRef.current = { shoulderHeight: currentShoulders, noseY: currentNoseY, noseX: currentNoseX };
    } else {
      baselineRef.current.shoulderHeight = (baselineRef.current.shoulderHeight + currentShoulders) / 2;
      if (face) {
        baselineRef.current.noseY = (baselineRef.current.noseY + currentNoseY) / 2;
        baselineRef.current.noseX = (baselineRef.current.noseX + currentNoseX) / 2;
      }
    }
  };
  ...
  const analyzeBody = (landmarks) => {
    ...
    setUiDebug(p => ({ ...p, posture: delta.toFixed(3), hand: activity.toFixed(0) }));
    metricsRef.current.handActivity = Math.round(activity);

    if (delta > 0.04) triggerAlert("POSTURE: SIT UP!", 2000);
    if (activity < 2 && metricsRef.current.words > 10) triggerAlert("HANDS: TOO STIFF", 4000);
  ...
  const analyzeFace = (landmarks, w, h, ctx) => {
    ...
    if(headHistoryRef.current.length > 5) {
      const mean = headHistoryRef.current.reduce((a,b)=>a+b, 0) / headHistoryRef.current.length;
      variance = headHistoryRef.current.reduce((a,b)=>a + Math.pow(b - mean, 2), 0);
    }
    const stability = Math.max(0, 100 - (variance * 20000));
    metricsRef.current.gazeStability = Math.round(stability);

    setUiDebug(p => ({ ...p, yaw: yawDrift.toFixed(2), pitch: pitchDrift.toFixed(2) }));
    metricsRef.current.readingScore = Math.max(0, 100 - Math.round(pitchDrift * 300));

    if (yawDrift > 0.15) triggerAlert("FACE: TURN TO CAMERA", 1500);
    if (pitchDrift > 0.12) triggerAlert("FACE: READING NOTES?", 1500);
    if (stability < 40 && metricsRef.current.words > 10) triggerAlert("FOCUS: HEAD MOVING", 2000);
  ...
  const triggerAlert = (msg, cooldown = 3000) => {
    ...
    if (now - lastTrigger > cooldown) {
      ...
      setAlerts(prev => {
        ...
        setTimeout(() => setAlerts(curr => curr.filter(a => a !== msg)), 2000);
        return newAlerts;
      });
    }
  };
  ...
  const handleSpeechResult = (event) => {
    ...
    if (interimContent) {
      ...
      if (FILLERS.includes(lastWord) && !alertCooldownRef.current[`filler-${lastWord}`]) {
  ...
  const drawTechSkeleton = (ctx, landmarks, color) => {
    const connections = [[11,12],[12,24],[24,23],[23,11]];
    ctx.lineWidth = 2;
  ...
  const generateLocalReport = (m, duration) => {
    ...
    if (m.wpm > 130) {
      grade = "A-"; title = "The Energetic Presenter";
      summary += "Your energy was high. ";
      tips.push("Pause for emphasis.");
    } else if (m.wpm < 100) {
      grade = "C+"; title = "The Thoughtful Observer";
...
const generateReport = async () => {
    ...
    const safeTranscript = fullTranscriptRef.current || "No speech recorded during this session.";
    
    // AI ANALYSIS
    let ai = { grade: "B", title: "Developing Speaker", summary: "Analysis complete.", tips: ["Practice makes perfect."] };
    try {
      const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
      const prompt = `Executive Coach Analysis. Stats: ${duration}m, ${m.wpm} WPM, ${m.fillers} fillers. Transcript: "${safeTranscript.substring(0,800)}". JSON { "grade": "S/A/B", "title": "Archetype", "summary": "Short summary", "tips": ["Tip 1", "Tip 2"] }`;
      const res = await model.generateContent(prompt);
      const text = res.response.text().replace(/```json/g, '').replace(/```/g, '');
      ai = JSON.parse(text);
    ...
    doc.text("SMART STANCE", 20, 20);
    doc.setFontSize(10); doc.setTextColor(...white);
    doc.text("AI COMMUNICATION ANALYSIS", 20, 30);
    ...
    doc.setFontSize(60); doc.setTextColor(...accent);
    ...
    // Tips Section
    y += 10;
    doc.setDrawColor(51, 65, 85); doc.line(20, y, 190, y);
    y += 15;
    ...
    doc.setTextColor(...accent); doc.setFontSize(16);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans overflow-hidden selection:bg-cyan-500/30">
      ...
      <AnimatePresence>
        {phase === 'boot' && (
          <motion.div exit={{ opacity: 0 }} className="flex h-screen items-center justify-center bg-black z-50">
            <div className="text-center relative">
              ...
              <div className="h-1 w-64 bg-slate-800 rounded-full overflow-hidden mx-auto">
                <motion.div className="h-full bg-cyan-500 box-shadow-[0_0_10px_#06b6d4]" 
                            initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ duration: 2 }} />
              </div>
              <p className="mt-4 font-mono text-cyan-500 text-sm tracking-[0.3em]">INITIALIZING NEURAL LINK...</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="flex h-screen items-center justify-center relative z-40">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} 
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white tracking-tight">SMART STANCE</h1>
                <p className="text-cyan-500 text-sm font-mono tracking-widest">PRO EDITION v2.0</p>
              </div>
            </div>

            <p className="text-slate-400 mb-8 leading-relaxed">
              Advanced AI communication analysis system ready. Ensure camera and microphone access for real-time telemetry.
            </p>
            ...
            <button onClick={() => setPhase('preview')} 
              ...
              <Target className="group-hover:rotate-180 transition-transform" /> ENGAGE SYSTEM
            </button>
          </motion.div>
        </div>
      )}
      ...
      {['preview', 'calibrating', 'live'].includes(phase) && (
        <div className="relative h-screen flex">
          ...
          <div className={`w-80 h-full z-20 p-6 flex flex-col gap-4 transition-transform duration-500 ${phase==='live' ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="p-4 rounded-xl bg-slate-900/60 backdrop-blur-md border border-slate-700/50 shadow-xl">
              <div className="flex items-center gap-2 text-cyan-400 mb-2 border-b border-white/10 pb-2">
                <Activity size={16} /> <span className="text-xs font-bold tracking-widest">PACING</span>
              </div>
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-mono font-bold text-white">{uiMetrics.wpm}</span>
                <span className="text-xs text-slate-400">WPM</span>
              </div>
              <div className="w-full bg-slate-800 h-1.5 mt-2 rounded-full overflow-hidden">
                <motion.div className="h-full bg-cyan-400" animate={{ width: `${Math.min(uiMetrics.wpm/2, 100)}%` }} />
              </div>
            </div>

            <div className="p-4 rounded-xl bg-slate-900/60 backdrop-blur-md border border-slate-700/50 shadow-xl">
              <div className="flex items-center gap-2 text-purple-400 mb-2 border-b border-white/10 pb-2">
                <AlertTriangle size={16} /> <span className="text-xs font-bold tracking-widest">FILLERS</span>
              </div>
              <span className="text-4xl font-mono font-bold text-white">{uiMetrics.fillers}</span>
            </div>

            <div className="mt-auto p-4 rounded-xl bg-slate-900/80 backdrop-blur-md border border-slate-700/50">
              <div className="flex items-center gap-2 text-green-400 mb-3">
                <Mic size={16} /> <span className="text-xs font-bold tracking-widest">AUDIO INPUT</span>
              </div>
              <canvas ref={audioCanvasRef} width={250} height={60} className="w-full" />
            </div>
          </div>
          ...
          <div className="absolute top-10 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center w-full max-w-lg pointer-events-none">
            <AnimatePresence>
              {alerts.map((msg, i) => (
                <motion.div key={i} initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <div className={`absolute right-0 top-0 h-full w-72 p-6 flex flex-col gap-4 transition-transform duration-500 ${phase==='live' ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="p-4 rounded-xl bg-slate-900/60 backdrop-blur-md border border-slate-700/50 shadow-xl space-y-4">
              <h3 className="text-xs font-bold text-slate-400 tracking-widest border-b border-white/10 pb-2">BIO-METRICS</h3>
              
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-300 flex items-center gap-2"><Eye size={14}/> FOCUS</span>
                <span className={`text-sm font-bold ${uiMetrics.gazeStability > 50 ? 'text-green-400' : 'text-red-400'}`}>{uiMetrics.gazeStability}%</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-300 flex items-center gap-2"><Smile size={14}/> WARMTH</span>
                <span className={`text-sm font-bold ${uiMetrics.smileScore > 50 ? 'text-green-400' : 'text-slate-400'}`}>{uiMetrics.smileScore}%</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-300 flex items-center gap-2"><Move size={14}/> GESTURE</span>
                <span className="text-sm font-bold text-cyan-400">{uiMetrics.handActivity}%</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-900/60 backdrop-blur-md border border-slate-700/50 shadow-xl mt-auto mb-20">
               <div className="flex items-center gap-2 text-cyan-400 mb-2">
                <Terminal size={14} /> <span className="text-[10px] font-bold tracking-widest">DEBUG STREAM</span>
              </div>
            ...
            {phase === 'live' ? (
               <div className="w-full max-w-4xl flex gap-6 items-end">
                  <div className="flex-1 bg-slate-900/50 backdrop-blur border border-white/10 rounded-lg p-3 h-20 overflow-hidden relative">
                    <p className="text-cyan-200/80 text-sm font-medium leading-relaxed">{transcript[0]} <span className="text-white bg-cyan-500/20">{transcript[1]}</span></p>
                  </div>
                  <button onClick={generateReport} className="px-8 py-4 bg-red-600 hover:bg-red-500 rounded-lg font-bold text-white shadow-lg shadow-red-900/50 transition-all flex items-center gap-2 shrink-0">
                    <Download size={20} /> END SESSION
                  </button>
               </div>
            ) : phase === 'preview' ? (
              <button onClick={() => setPhase('calibrating')} className="px-12 py-5 bg-cyan-500 text-slate-900 font-bold text-xl rounded-full shadow-[0_0_30px_rgba(6,182,212,0.6)] animate-pulse">
                START CALIBRATION
              </button>
            ) : null}
          </div>
          ...
          {phase === 'calibrating' && (
             <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
               <motion.div key={countdown} initial={{ scale: 2, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
                 <div className="text-[12rem] font-bold text-transparent bg-clip-text bg-gradient-to-b from-cyan-300 to-cyan-600 leading-none">{countdown}</div>
                 <p className="text-white text-xl font-bold tracking-[0.5em] mt-4">CALIBRATING SENSORS</p>
      ...
      {phase === 'processing' && (
        <div className="flex h-screen items-center justify-center bg-slate-900 z-50">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">COMPILING REPORT</h2>
            <p className="text-slate-400 animate-pulse">{reportStatus === 'analyzing' ? 'CONSULTING AI NEURAL NET...' : 'GENERATING SECURE PDF...'}</p>
          </div>
        </div>
      )}
    </div>
